...
const acaoBadge = (a: string) => {
  const map: Record<string, { label: string; color: string }> = {
    envio: { label: "Envio", color: "bg-blue-500/10 text-blue-600 border-blue-200" },
    baixa: { label: "Baixa", color: "bg-green-500/10 text-green-600 border-green-200" },
    cancelamento: { label: "Cancelamento", color: "bg-red-500/10 text-red-600 border-red-200" },
    confirmacao: { label: "Confirmação", color: "bg-purple-500/10 text-purple-600 border-purple-200" },
...
export default function GestorSPCSerasa({ onBack }: GestorSPCSerasaProps) {
  ...
  const statCards = [
    ...
    { label: "Envios Pendentes", value: stats.pendentes, icon: Clock, color: "text-yellow-500", bg: "bg-yellow-500/10" },
    { label: "Negativados Ativos", value: stats.negativados, icon: AlertTriangle, color: "text-red-500", bg: "bg-red-500/10" },
    { label: "Baixas Realizadas", value: stats.baixas, icon: CheckCircle, color: "text-green-500", bg: "bg-green-500/10" },
  ];
