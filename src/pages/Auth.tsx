...
export default function Auth() {
  ...
  return (
    ...
    >
      {/* ESQUERDA — 57% — cards centralizados entre logo (53%) e rodapé (92%) */}
      <div
        className="hidden lg:block"
        style={{ width: '57%', position: 'relative' }}
      >
        {/* Container dos cards posicionado absolutamente na faixa 53%-92% */}
        <div
          style={{
            position: 'absolute',
            top: '53%',
        ...
        >
          <div style={{ width: '100%', maxWidth: '400px' }}>
            ...
            {/* Card suporte — metade da largura */}
            <div className="mt-2.5" style={{ width: '48%' }}>
              <div
                className="rounded-xl p-3 space-y-1.5"
                style={{
                  backgroundColor: 'rgba(5,15,35,0.72)',
      ...
      >
        ...
        >
          <div className="space-y-1">
            <h2 className="text-xl font-bold tracking-tight" style={{ color: '#1a1a2e' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {!isLogin && (
              <div className="space-y-1">
                ...
                <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Seu nome" required className="h-10 bg-white border-gray-200 text-gray-900 text-sm" />
              </div>
            )}
            <div className="space-y-1">
              <Label className="text-xs font-medium text-gray-600">E-mail</Label>
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="seu@email.com" required className="h-10 bg-white border-gray-200 text-gray-900 text-sm" />
            </div>
            <div className="space-y-1">
              ...
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required minLength={6} className="h-10 bg-white border-gray-200 text-gray-900 text-sm" />
            </div>
            <Button
              type="submit"
              className="w-full h-10 font-semibold text-white text-sm"
